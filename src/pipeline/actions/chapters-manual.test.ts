...
const segments: WhisperSegment[] = [
  makeSegment(
    0,
    30,
    "Welcome to the podcast. Today we have a great episode planned.",
  ),
  makeSegment(
    30,
    60,
    "I'm your host and we're going to dive into machine learning.",
  ),
  ...
  makeSegment(
    210,
    240,
    "Now let's move on to transformers, a breakthrough architecture.",
  ),
  ...
  makeSegment(
    300,
    330,
    "The key innovation was the attention is all you need paper.",
  ),
  makeSegment(330, 360, "Let's talk about practical applications now."),
  makeSegment(
    360,
    390,
    "Large language models like GPT are built on transformers.",
  ),
  makeSegment(
    390,
    420,
    "They can generate text, answer questions, and write code.",
  ),
  ...
  makeSegment(
    480,
    510,
    "They're being used in medical imaging and autonomous driving.",
  ),
...
describe.skipIf(!process.env.MANUAL_TEST)("manual", () => {
  test("chapter system prompt produces parseable output", async () => {
    const prompt = buildChapterPrompt(
      segments,
      "Generate chapters for this podcast transcript.",
    );
